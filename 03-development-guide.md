...
- **Do not reintroduce rejected designs.** Document 1 §6 lists 13 decisions with their rejected alternatives. If a simpler-looking path contradicts one (naive FIFO, model-as-unit, hard-default preemption, weights-only fit, in-process engines, Docker Mac workers, …), it was already ruled out — don't "improve" back into it.
...
The phase order follows the standard dependency spine: scaffold → contracts/types → test infra → most-depended module (the scheduler core) → its consumers (node agent, gateway) → integrations → onboarding → optimizer. Real specialty hardware appears only at phase-boundary smoke gates; everything else is built and verified on the local dev Mac against the mock fleet.
...
1. **Scaffold** the repo tree from Document 1 §5: `go.mod` (`module mycelium`, Go 1.23), `cmd/mycelium/` with a no-op `main.go` that dispatches `server`/`node`, and empty package dirs under `internal/` and `test/`.
2. **`internal/domain`** — all types and enums and typed errors from Document 2 §2 (`Job`, `Node`, `Accelerator`, `Preset`, `Claim`, `ModelInstance`, `Lease`, `Reservation`, `PlacementDecision`, `FleetSnapshot`, `NodeSnapshot`, `RunMetric`, `Project`, the `domain.Err*` set). No logic, std-lib only.
3. **`internal/ports`** — every interface from Document 2 §2 (`Clock`, `ResourceEstimator`, `Allocator`, `BackendAdapter`, `NodeAgent`, `Placer`, `TelemetrySink`, plus stubs for `ModelRegistry`, `Catalog`, `TelemetryStore`, `Optimizer`, `Discovery`, `Tunnel`, `Store`).
...
8. **`internal/scheduler`** — the brain: `filter` → `selector` (speed-pref-aware) → `scorer` → `scheduler` (admit/queue) → `preempt` (soft default; hard ladder; fit-forced reallocation = same priority test, candidate set pinned to the one unit). Implements `ports.Placer`. 100% coverage required.
...
**Goal:** make a real machine do work. Build the per-node lifecycle, the first real backend adapter, and wire the telemetry substrate from the start (it must emit before the optimizer exists). Validate on the local dev Mac (llama.cpp Metal) and the two second peers as real remote nodes.

**Depends on:** Phase 0 (ports, mocks, scheduler).

**Build:**

1. **`internal/node`** — `agent.go` (owns lifecycle, sends heartbeats via injected `Clock`, enforces leases locally), `process.go` (spawn/stop via `cmd`/`cmdStop`, readiness gate — no routing until health passes, cold-start dedup so two requests for the same cold preset wait on one load), `instance.go` (per-instance state machine), `loadingstate.go` (SSE loading-state during cold load), and a **startup reaper** that cleans up orphaned backend processes/containers left by a previous (crashed) run so none keep holding VRAM (Doc 1 §3.10 — llama-swap lacks this; build it in). The agent **sheds** (429-style) when saturated rather than building a local queue — all queueing lives in the control plane. Implements `ports.NodeAgent`. *(Parallel-OK with step 2.)*
...
## Phase 4 — Membership and onboarding

**Goal:** "a node joins the hive." One command on a new machine and it shows up, reachable, in the server's node list.

**Depends on:** Phase 1 (a node agent to onboard).

**Build:** `internal/membership/` — `token.go` (shared join token), `discovery_lan.go` (LAN discovery — MVP), `tunnel.go` (loopback tunnel allocation). `discovery_overlay.go` (libp2p, cross-NAT) is a **roadmap** stub behind the same `Discovery` interface — leave it unimplemented but interface-shaped, not half-built.

**Your call (Phase 4):**
- *LAN discovery mechanism.* mDNS, UDP broadcast, a seed address — your pick, behind `ports.Discovery`. Hard requirement: a `mycelium node --join <token>` started on the same LAN appears in the server's node list and is reachable via an allocated loopback tunnel, with no manual address wrangling.
- *Join-token handling.* The token gets a node into the fleet; it is membership, not per-backend auth (LocalAI's "P2P token ≠ service auth" lesson). Support rotating and revoking the token so a leaked one can be invalidated without re-onboarding every node, and don't treat token possession as authorization for anything beyond joining.

**Gate (Phase 4):**

```bash
go test ./internal/membership/... -race
go test ./... -race
```

Plus a smoke check on a **second real machine** (a second peer): `mycelium node --join <token>` brings it online in one command, the server lists it as `ready`, and a job places and runs on it through the normal scheduler path.
...
## Roadmap (post-MVP — not phased here)
...
- **libp2p overlay** — implement `discovery_overlay.go` behind the existing `Discovery`/`Tunnel` interfaces for cross-NAT membership.
...
- **Cross-machine model sharding** — a unit spanning multiple nodes for pipeline-parallel inference (MLX-distributed across several Macs: hostfile generation + rank coordination). The resource model already calls a unit "a set of accelerators"; extending that across machines is the hook. Overlaps the standalone distributed-Apple-inference effort.
...
| 1 | node agent + llama.cpp + telemetry + reactive requeue | smoke only | smoke on local dev Mac + 2 second peers: real load/serve/stop, real metric, real requeue |
...
| 4 | membership + onboarding (token + LAN) | smoke only | `mycelium node --join` brings a 2nd real machine online in one command |
| 5 | optimizer (proactive recommend + gated auto-apply) | none | shift → correct recommendation; auto-apply respects the toggle |

Build Phase 0 first and completely. It is the part nobody else has built, and it is the part you can finish on the local dev Mac with nothing plugged in.
